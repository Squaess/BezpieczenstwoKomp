...
void setup(int keySize) {
	...
	char command[150];
	sprintf(command, "keytool -genkeypair -keystore ./serv.keystore -alias alias -storepass %s -keyalg RSA -keysize %d", passwd.c_str(), keySize);
...
bool check(string msg) {
	...
	char conf[150];
	sprintf(conf, "keytool -list -v -keystore ./serv.keystore -storepass %s", msg.c_str());
...
void signService(int clientFD) {
	...
	if(check(msg)){
		...
		if(BN_is_one(gcd)){
			...
			BIGNUM *re = BN_new();
			BN_mod_exp(re, r, e, n, ctx);

			BIGNUM *rx = BN_new();
			BN_mod_mul(rx, re, x, n, ctx);

			BIGNUM *y = BN_new();
			BN_mod_exp(y, rx, d, n, ctx);

			BIGNUM *rn = BN_new();
			BN_mod_inverse(rn, r, n, ctx);
			BN_mod_mul(x, y, rn, n, ctx);

			string send = string(BN_bn2dec(x));
			//cout << "#####################\n";
			//cout << send << endl;
